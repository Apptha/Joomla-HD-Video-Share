...
class mod_hdvideosharecategoriesInstallerScript {
  ...
  public function install($parent) {
    ...
    $query->update ( $db->quoteName ( '#__modules' ) )->set ( $db->quoteName ( 'published' ) . ' = ' . $db->quote ( '1' ) )->where ( $db->quoteName ( 'module' ) . ' = ' . $db->quote ( 'mod_hdvideosharecategories' ) );
    $db->setQuery ( $query );
    $db->query ();
    
    /** Query to get categories module id */
    $query->clear ()->select ( 'id' )->from ( '#__modules' )->where ( $db->quoteName ( 'module' ) . ' = ' . $db->quote ( 'mod_hdvideosharecategories' ) );
    $db->setQuery ( $query );
    $db->query ();
    $mid4 = $db->loadResult ();
